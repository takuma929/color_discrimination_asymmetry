...
for iCond = 1:numel(conditions)
    ...
    if doSave
        pause(0.1)
        nameMain = sprintf('figS2a_main_%s.pdf', thisCond.tag);
        exportgraphics(figMain, fullfile(outdir, nameMain), ...
            'ContentType', 'vector', 'BackgroundColor', 'none');
        fprintf('%s successfully saved.\n', nameMain);
        nameTask = sprintf('figS2b_color_task_%s.pdf', thisCond.tag);
        exportgraphics(figTask, fullfile(outdir, nameTask), ...
            'ContentType', 'vector', 'BackgroundColor', 'none');
        fprintf('%s successfully saved.\n', nameTask);
    end
end
