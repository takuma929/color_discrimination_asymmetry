%% figS8_taskonomy_24tasks_layer.m
...
% figS7_taskonomy.m. Tasks are then ordered from early (Layer 1) to deep
...
%
% Requires the Statistics and Machine Learning Toolbox (groupsummary / unstack).
%
...
% SE across iterations). The grid is organised by best-aligned layer.
...
% early (Layer 1) to deep (Block 4) and coloured by that layer, using the
...
if doSave
    pause(0.1)
    exportgraphics(fig, fullfile(outdir, 'figS8_taskonomy_24tasks_layer.pdf'), ...
        'ContentType', 'vector', 'BackgroundColor', 'none');
    fprintf('%s successfully saved.\n', 'figS8_taskonomy_24tasks_layer.pdf');
    exportgraphics(fig, fullfile(outdir, 'figS8_taskonomy_24tasks_layer.png'), ...
        'ContentType', 'image', 'BackgroundColor', 'none', 'Resolution', 600);
    fprintf('%s successfully saved.\n', 'figS8_taskonomy_24tasks_layer.png');
end
...
function [order, labels, colors] = layerPalette()
    ...
    % the first stage, drawn black in the architecture figure) is shown in GRAY here.
...
function [profiles, layerId] = assignBestAlignedLayer(profiles, humanMean, depthOrder)
    ...
    % layer from early (Layer 1) to deep (Block 4). Returns the reordered
...
function fig = plotTaskonomy24TaskGridByLayer(profiles, layerId, layerColors, ...
    ...
    depthOrder = ["layer_1", "block_1", "block_2", "block_3", "block_4"];
...
function P = taskonomyLayerProfiles(taskonomy_csv, selectedTasks)
    ...
    % every depth (averaged across iterations), kept in layer_1 -> block_4 order.
    assert(isfile(taskonomy_csv), 'Taskonomy CSV not found: %s', taskonomy_csv);
