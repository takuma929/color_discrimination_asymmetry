...
function [xVals, yVals] = decodingAccuracyLogOddsForExperiment(dec, expIdx, timeWindow, stepLevel)
    ...
    % Convert averaged accuracies to log10 odds ratios. Accuracy is clipped away
    % from 0 and 1 so the odds transform remains finite.
...
function vals = extractParticipantAccuracy(A, refDim, axisDim, participantDim, refIdx, axisIdx)
    ...
    % pair. All dimensions except participant are either selected or averaged.
